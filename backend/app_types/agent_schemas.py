...
class CommunicationAnalysis(BaseModel):
    ...
    communication_score: int = Field(
        ge=0, le=100, description="Communication quality score (0-100)"
    )
...
class ConfidenceAnalysis(BaseModel):
    ...
    confidence_level: ClarityLevel = Field(description="Overall confidence level")
    emotional_tone: EmotionalTone = Field(description="Detected emotional tone")
...
class PersonalityAnalysis(BaseModel):
    ...
    overall_summary: str = Field(
        min_length=10, description="Professional summary of personality analysis"
    )
